...
const DEMO_LINKS = {
  "ai-tycoon-rialo-game": {
    url: "https://romeototo.github.io/ai-tycoon-rialo-game/",
    label: "Play Game",
    icon: "play_arrow",
  },
  "monster-tapper": {
    url: "https://romeototo.github.io/monster-tapper/",
    label: "Play Game",
    icon: "play_arrow",
  },
  "portfolio-website": {
    url: "https://romeototo.github.io/portfolio-website/",
    label: "View Site",
    icon: "open_in_new",
  },
  "telegram-ai-it-automation-agent": {
    url: "https://github.com/romeototo/telegram-ai-it-automation-agent",
    label: "View Repo",
    icon: "code",
  },
  resume: {
    url: "https://romeototo.github.io/resume/",
    label: "View CV",
    icon: "article",
  },
};
...
async function fetchGitHub(forceRefresh = false) {
  try {
    ...
    if (!user || !repos) {
      ...
      if (!userRes.ok || !repoRes.ok) {
        ...
        localStorage.setItem(
          CACHE_KEY,
          JSON.stringify({ timestamp: Date.now(), user, repos }),
        );
      }
    }

    document.getElementById("ghAvatar").src =
      user.avatar_url || `https://github.com/${GH_USER}.png`;
    ...
    if (repos.length > 0) {
      const last = [...repos].sort(
        (a, b) => new Date(b.pushed_at) - new Date(a.pushed_at),
      )[0];
      const diff = Math.floor(
        (Date.now() - new Date(last.pushed_at)) / 86400000,
      );
...
function renderRepos(repos) {
  ...
  repos.forEach((repo) => {
    ...
    const playBtn = demo
      ? `<a href="${demo.url}" target="_blank" class="repo-play" onclick="event.stopPropagation()"><span class="material-icons">${demo.icon}</span>${demo.label}</a>`
      : "";
    const desc =
      repo.description || DESC_FALLBACK[repo.name] || "No description provided";
    item.innerHTML = `
...
function loadXStats() {
  ...
  if (d.followers)
    document.getElementById("xFollowers").textContent = d.followers;
  if (d.posts) document.getElementById("xPosts").textContent = d.posts;
  if (d.following)
    document.getElementById("xFollowing").textContent = d.following;
...
function saveGoal() {
  ...
  const target =
    parseInt(document.getElementById("goalTargetInput").value) || 7;
...
function updateWeeklyGoalProgress(repos) {
  ...
  const weekCommits = repos.filter(
    (r) => r.pushed_at && new Date(r.pushed_at) > weekAgo,
  ).length;
...
document.getElementById("editGoalBtn").addEventListener("click", () => {
  ...
  const saved = localStorage.getItem("rmt_goal");
  if (saved)
    document.getElementById("goalInput").value = JSON.parse(saved).text || "";
});
...
function saveTodos() {
  localStorage.setItem("rmt_todos", JSON.stringify(todos));
}
